"""Pure builders for the four standardized game JSON artifacts (Appendix F):
...
from thief_agent.report.artifact_schemas import (
    DEFAULT_TIMEZONE,
    SCHEMA_CONFIG,
    SCHEMA_DECLARATION,
    SCHEMA_LOG,
...
def build_log(summary: dict, game_id, game_uid, group_id, opponent_group_id) -> dict:
    """Template 3: this peer's per-sub-game commit-reveal log, from its runtime summary."""
...
def build_result(
    game_id, game_uid, group_ids, sub_games: list, aggregate_out: dict, mutual_sha256: str
) -> dict:

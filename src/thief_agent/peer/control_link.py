...
from thief_agent.domain.protocol import ControlMessage
...
class ControlLink:
    ...
    def __init__(self, role: str, transport, controls, listener=None):
        ...
        self._opponent: dict[str, Any] = {
            "status": "-",
            "sub_game_number": None,
            "step_budget": None,
        }
        self._last_status_key: tuple[str, float] | None = None
    ...
    def broadcast_status(self, status: str, sub_game_number: int, step_budget: float) -> None:
        ...
        self._send(
            "status", status=status, sub_game_number=sub_game_number, step_budget=step_budget
        )

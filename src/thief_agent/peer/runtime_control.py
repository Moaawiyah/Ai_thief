...
from thief_agent.peer.control_link import (
    GAME_OVER,
    PAUSED,
    PLAYING,
    QUIT,
...
def pump(rt, base: str) -> None:
    ...
    if budget is None:
        budget = rt.config.get("llm.step_deadline_seconds") or 0.0
    rt.link.broadcast_status(_status_for(rt, base), rt.sub_game_number, budget)
